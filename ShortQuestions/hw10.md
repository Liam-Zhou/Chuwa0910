...
`@OneToMany`, `@ManyToOne`, and `@ManyToMany` are common annotations <u>used in JPA (Java Persistence API) and Hibernate to specify one-to-many, many-to-one and many-to-many relationships between entities</u>. 
...
<u>all operations (persist, merge, remove, refresh) should be cascaded to related entities.</u>

`orphanRemoval = true`:

<u>when the association between entity A and entity B is removed and entity B is no longer referenced elsewhere, entity B will also be deleted.</u>

当entityA和entity B之间的关系解除，并且实体B不再在其他地方被引用时，实体B也将被删除



**what are the other CascadeType and their features?**
- `CascadeType.ALL`: 
   - all operations (persist, merge, remove, refresh) should be cascaded to related entities
...
<u>when a record is fetched from the database, all the associated records from related tables are also fetched.</u>
   - for example: when we fetch post, JPA also fetch comment at the same time

`FetchType.LAZY`： 

<u>fetches the records only when they are needed.</u>
   - If we don't need data from comment, JPA doesn't fetch it, Only fetch data from post
   - for example: when we need data from comment, then JPA fecth comment data
...
It is a query language used to <u>write more complex queries in JPA that cannot be expressed using simple CRUD operations</u>. 

JPQL is similar to SQL (Structured Query Language) but operates on entities and their properties rather than database tables and columns.

While JPA provides easy-to-use CRUD (Create, Read, Update, Delete) operations through its EntityManager, JPQL (Java Persistence Query Language) is used for more complex queries that cannot be expressed using simple CRUD operations. 
...
both HQL and Criteria Queries are powerful tools in Hibernate for querying databases.

HQL (Hibernate Query Language) and Criteria Queries are <u>two approaches to querying a relational database using Hibernate.</u> 

They provide alternatives to standard SQL queries when working with Hibernate.
和standard SQL查询作用一样，但是是hibernate提供的。




## 18. What is EnityManager?(JPQL里面的)

the EntityManager <u>acts as a bridge between Java application code and the underlying database.</u>

It is <u>an interface in JPQL that manages entity persistence, provides transactional support, executes queries, and controls the lifecycle of entities within a persistence context.</u>

JPA的话用repository作为桥梁，JPQL用EnityManager做桥梁执行对数据库的查询

EntityManager 允许创建并执行基于 JPA 的查询。通常，这些查询使用 Java Persistence Query Language（JPQL）编写，通过 EntityManager 对象执行。
...
桥梁： entity和persistence(databse)之间执行CRUD operation.

hibernate的桥梁是session，JPQL的桥梁是entitymanager，JPA的桥梁是repository
...
2. using transaction APIs provided by the database or ORM framework
   - using the Connection object in JDBC. 
      - can start a transaction using :connection.setAutoCommit(false)
   - use the EntityManager to manage transactions.  
      - typically begin a transaction with entityManager.getTransaction().begin() (EntityManager / session)
...
1. **`@Transactional`**:
   ...
   - Spring takes care of transaction demarcation, propagation, and error handling. You can configure transaction management using XML or Java configuration.
...
2. **Connection object (JDBC)**:
   - When working with raw JDBC, you can manually manage transactions using the Connection object. 
   - You can start a transaction using connection.setAutoCommit(false), perform database operations, and then commit the transaction using connection.commit(). If an exception occurs, you can roll back the transaction using connection.rollback().
...
3. **EntityManager(JPA)**:
   - When using JPA with a JPA provider like Hibernate or EclipseLink, can use the EntityManager to manage transactions. 
   - typically begin a transaction with entityManager.getTransaction().begin(), perform database operations, and then either commit or roll back the transaction using entityManager.getTransaction().commit() or entityManager.getTransaction().rollback(.
...
Hibernate caching is <u>a mechanism to stort frequently accessed data in memory. 

It include 3 types of caching: first-level cache(session cache), second-level cache, and query cache.</u>

**First-Level Cache (Session Cache)**: <u>to store entity is queried in a session, there is no need to git the database again if the same entity in the same session is queried.</u>

It reduces the number of SQL queries by caching the objects retrieved within a session

**Second-Level Cache**: <u>is shared among different sessions to store data that is frequently accessed.</u>
...
![hibernate Caching](../images/hw10/hibernate%20caching.png)
...
@Transactional is <u>an annotation</u> in Spring <u>to manage the transcation, represents a unit of work. if one step fails, the whole transaction fails</u>.
