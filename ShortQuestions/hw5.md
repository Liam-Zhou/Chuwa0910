...
Callable is <u>an interface to define a task could execute with multiple threads.</u>

1. return type
- runnable has no return.
- callable has return.

2. method to define a task
...
2. time
- t.start(): <u>One can call the start() method only once.</u>  只能对一个线程对象调用 start() 方法一次(会初始化一个新的系统级线程，并在该线程中执行相关的 run() 方法。)
- t.run(): <u>One can call the run() method multiple times.</u>  run() 方法的每次调用都是在当前线程（通常是主线程）中执行，而不会创建新的线程。这意味着它会按照顺序执行 run() 方法，而不是并发执行。
...
new -> runnabel -> blocked -> waiting -> timed_waiting -> dead/terminated

A thread can have one of the following states during its lifetime:
1. `New`: 
- <u>a Thread class object is created but doesn't start until call the start() method.</u>
...
- This state occurs when <u>a thread attempts to access a synchronized block that is already held by another thread.</u>
...
- This state occurs when <u>a thread is waiting, without a timeout, for another thread to perform a particular task.</u> 
- 进入该状态的线程需要等待其他线程做出一些特定动作（通知或中断）。处于这种状态的线程不会被分配CPU执行时间，它们要等待被显式地唤醒，否则会处于无限期等待的状态

5. `Timed_waiting`:
- <u>A thread is waiting for another thread to perform a specific action, but with a timeout specified.</u>
...
1. Usage Context:
- wait() <u>release the lock it holds on that object and enters a waiting state.</u> 当线程调用对象上的wait（）时，它释放该对象的锁并进入等待状态。
- sleep() <u>pause the execution of a thread for a specified amount of time.</u> 在指定的时间内暂停线程的执行

2. Method defined:
- wait() <u>is defined in object instance</u> and releases the lock on the object, allowing other threads to access the synchronized block or method.  在对象实例上调用wait（），并释放对象上的锁，允许其他线程访问同步块或方法。
- sleep() <u>is defined in Thread class</u> and doesn't release any locks held by the thread. 直接在线程类上调用，不会释放线程持有的任何锁
...
Deadlock is <u>a situation that every thread is waiting for a resource which is held by some other waiting thread</u>.  多个线程blocked
...
The Executor Framework in Java, introduced in the java.util.concurrent package, provides a high-level abstraction <u>for managing and executing tasks asynchronously using thread pool</u>. 

It's designed to simplify concurrent programming by decoupling task submission from task execution(通过解耦提交任务+执行任务，来简化并发编程), <u>offering various types of executors to suit different task execution strategies.</u>（提供不同类型的executors来适应不同的任务执行策略）
...
Creating executors using the Executor Framework involves using the java.util.concurrent package classes (ThreadPoolExecutor, Executors) to <u>create instances of ExecutorService or ScheduledExecutorService.</u>  

1. `ThreadPoolExecutor`: 可配置的thread pool
...
2. `FixedThreadPool`:  固定线程数的thread pool
...
3. `CachedThreadPool`: 动态调整线程数的thread pool
...
4. `SingleThreadExecutor`: 只包含一个thread的thread pool
...
- shutdown():  
    - <u>allows the threads in the thread pool to finish executing the tasks that they have already started or are currently executing.</u> 
    - 允许线程池中的线程完成已经开始或正在执行的任务。
- shutdownNow():
    - attempts to abruptly shut down the ExecutorService by <u>interrupting the executing threads, stops the processing of waiting tasks, and returns a list of the tasks that were awaiting execution.</u>
    - 试图立即停止ExecutorService，中断执行线程，停止处理等待的任务，并返回正在队列中等待执行的任务列表。
...
Future is <u>a interface gives the result of an asynchronous computation.</u>

It provides methods like <u>get(), cancale(), isDone()</u> to check if the computation is completed, cancel the computation, and retrieve the result (if available).

CompletableFuture <u>offers more features than Future interface</u> for handling and chaining multiple asynchronous computations with more control and flexibility

CompletableFuture provides various methods like <u>thenApply(), thenAccept(), thenCompose(), exceptionally(), handle(), etc.</u>, allowing developers to chain asynchronous computations and handle results or errors more flexibly.

**give me the example in your experience:**

For the process of quering order, make payment and send email:

- single thread: All operations are executed within a single thread, requiring the completion of all three stages for the first order before proceeding to the second one. If operations such as querying orders  block, it will take a longer time.

- Future version: Initially, concurrent processing of query operations for all orders occurs, followed by concurrent handling of all makePayment operations, and then concurrent processing of sendEmail operations (queries, payments, and sending occur in stages).

- CompletableFuture version: Allows for concurrent processing of queries, payments, and email sending for multiple orders simultaneously.


batch_payment:

- 单线程版本：所有操作都在一个线程中执行，需要执行完第一个订单的全部3个流程才能够执行第二个订单。查询order，支付和发送邮件等操作如果阻塞，会需要较长时间
    ![单线程版本](../images/hw5/%E5%8D%95%E7%BA%BF%E7%A8%8B%E7%89%88%E6%9C%AC.png)

- Future版本：先并发处理所有order的query，再并发处理完所有makePayment，再并发处理sendEmail(查询，支付，发送是分阶段进行的)
    ![future版本](../images/hw5/future%E7%89%88%E6%9C%AC.png)

- CompletableFuture版本：允许同时并发处理多个订单的查询，支付和发送邮件。
...
**find in Coding file coding - hw5-multithreading:learn**
