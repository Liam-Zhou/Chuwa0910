...
**find in coding file: hw4-generic**

https://github.com/TAIsRich/chuwa-eij-tutorial/tree/main/02-java-core/src/main/java/com/chuwa/tutorial/t01_basic/generic

Generics <u>allow to create classes, interfaces, and methods without giving a specific data type until the code is actually used.</u> 
...
![method reference](../images/hw4/method%20reference%20code.png)
...
**find in coding file: hw4-stream_api**
...
**find in coding file: hw4-java8: exercise**

https://github.com/B1gO/chuwa-eij-tutorial/blob/main/02-java-core/src/main/java/com/chuwa/tutorial/t06_java8/exercise/ShoppingCartUtil.java
...
1. Null-Safety:
    ...
    It is designed to <u>avoid Null checks and run NullPointerExceptions.</u>

2. Expressive API:

    Optional <u>provides methods like isPresent(), ifPresent(), orElse(), and orElseGet() to work with optional values in a more readable and concise manner.</u>
...
It is <u>an annotation to indicate an interface is a functional interface that has only one abstract method.</u>


Functional interfaces:
- Has only one abstract method  只包含一个抽象方法的接口，称为函数式接口
- @FunctionalInterface - for sanity check     在一个接口上使用 @FunctionalInterface 注解，这样做可以检查它是否是一个函数式接口。
- Lambda is the implementation of the abstract method. 可以通过 Lambda 表达式来创建该接口的对象。
...
a lambda expression is a concise way to <u>replace anonymous inner class and work with functional interface to create instance of functional interafce.</u>
...
Method reference is used to <u>refer methods or constructors by using the double colon :: operator.</u>

- Java 8 Method reference is used to refer method of functional interface.  方法引用是用来 直接访问 类或者实例的 已经存在的 方法或者构造方法。
...
![method reference](../images/hw4/method%20reference%20.png)
...
![筛选与切片](../images/hw4/%E6%8E%92%E5%BA%8F.png)

2. 映射

![映射](../images/hw4/%E6%98%A0%E5%B0%84.png)

3. 排序

![排序](../images/hw4/%E6%8E%92%E5%BA%8F.png)
...
![匹配与查找](../images/hw4/%E5%8C%B9%E9%85%8D%E4%B8%8E%E6%9F%A5%E6%89%BE.png)

2. 归约

![归约](../images/hw4/%E5%BD%92%E7%BA%A6.png)

3. 收集

![收集](../images/hw4/%E6%94%B6%E9%9B%86.png)
...
<u>Yes, by variable capture.</u>

<u>lambda can use effectively final variables, non-final variables(instance variables and static variable) and method parameters by the ability of variable capture.</u>
...
- `Default method and static method in interface`:
    -  method that provides a default implementation, add static method into interface
- `Lambda and Functional Interface` : 
    - replace anonymous inner class and work with functional interface to create instance of functional interafce.
    - an interface that only has one abstract class.
- `Method references`: 
    - refer methods or constructors by using the double colon :: operator.
- `Optional class`:
    - deal with NullPointerException
- `Stream API`:
    - a way to process collections of data, and replace traditional iterative loops by using functional-style operations
...
When you extend an interface to create a functional interface, you <u>ensure that the extended interface has only one abstract method</u> (the one required for it to be considered a functional interface).
...
Lambda expressions:
- To replace anonymous inner class, 以前用匿名实现类表示的现在都可以用 Lambda 表达式来写。
- Work with functional interface.
- lambda 表达式的本质：an instances of functional interfaces 作为函数式接口的实例 
- Lambda表达式: https://juejin.cn/post/6962035387787116551#heading-0
...
It is an example of method reference to <u>call the valueOf method from String class.</u>
...
**Intermediate operations** <u>are operations that transform one stream into another but do not produce a final result. </u>

**Terminal operations** <u>are operations that produce a final result or a side effect.</u>
...
2. Iteration
    - Collections: external iteration: manual iteration
    - Streams: internal iteration: automatic iteration
3. element access
