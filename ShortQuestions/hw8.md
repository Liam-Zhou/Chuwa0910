...
3. 在请求的 Headers 部分添加 `Content-Type`，将其值设置为 `application/json`。
4. 在请求的 body 部分选择 `raw`，并选择 `JSON` 格式。
5. 在 body 中编写 GraphQL 查询。例如：

```json
{
    // 查询会请求从 ID 为 1 的文章中获取 `id`、`title`、`description` 和 `content` 字段。
    "query": "{ getPostById(id: 1) { id, title, description, content } }"
}
```
6. 点击发送按钮来执行请求，并查看返回的结果。

对于执行创建文章的 Mutation，您可以在请求 body 中编写类似以下的内容：

```json
{
    // 创建一篇标题为 "Sample Title"，描述为 "Sample Description"，内容为 "Sample Content" 的文章，并返回创建后的文章的相关字段。
    "query": "mutation { createPost(title: \"Sample Title\", description: \"Sample Description\", content: \"Sample Content\") { id, title, description, content } }"
}
```
