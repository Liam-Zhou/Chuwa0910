...
String pool is the space reserved in the heap memory that can be used to <u>store the strings</u>.
...
1. basic
    - overloading: a polymorphism technique to <u>create multiple methods with the same name but different signature</u>.
...
2. **Linking**:
    ...
    **Preparation**: 
    <u>Static variables (class variables) in the class are allocated memory and initialized with their default values</u>(为static 和 class variables分配内存并且赋值). These variables are initialized to zero, null, or appropriate default values depending on their data types.
