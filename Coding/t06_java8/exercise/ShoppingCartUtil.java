...
public class ShoppingCartUtil {
     ...
     */
    //public static Optional<String> getFirstItemName(Customer customer) { return null; }
    ...
    public static String getFirstItemNameOrThrowException(Customer customer) throws EmptyCartException {
        return Optional.ofNullable(customer)
                ...
                .map(items -> items.get(0))
                .map(Item::getName)
                .orElseThrow(() -> new EmptyCartException("shopping cart is empty")); //() no parameter
        //return null;
    }
    ...
    public static void checkItemsInCart(Customer customer) {
        // write you code
        Optional<List<Item>> itemsOptioanl = Optional.ofNullable(customer)
                .map(Customer::getShoppingCart)
                .map(ShoppingCart::getItems);

        //itemsOptional.isPresent() check ensures that the Optional itself is not empty or null.
        // It confirms that the Optional contains a value.
        // !itemsOptional.get().isEmpty() check ensures that the value inside the Optional is not an empty list.
        // Even if the Optional is present, it could contain an empty list,
        if(itemsOptioanl.isPresent() && !itemsOptioanl.get().isEmpty()) {
    ...
    public static void printItemsInCart(Customer customer) {
        ...
        // isPresent is used to check if an Optional contains a value
        // ifPresent is used to perform an action on the value inside the Optional when it is present.
