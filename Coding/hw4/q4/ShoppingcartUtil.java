...
public class ShoppingCartUtil {
    ...
    public static void checkItemsInCart(Customer customer) {
        Optional<List<Item>> item = Optional.ofNullable(customer)
                .map(Customer::getShoppingCart)
                .map(ShoppingCart::getItems);
        if(item.isPresent() && !item.isEmpty()){
            System.out.println("have goods in shopping cart");
        }else{
            System.out.println("empty shoppint cart");
        }
    }
    ...
    public static void printItemsInCart(Customer customer) {
        ...
        items.ifPresent(item -> {
            if(!item.isEmpty()){
                item.forEach(itemm -> System.out.println(itemm.getName()));
            }
        });
    }
