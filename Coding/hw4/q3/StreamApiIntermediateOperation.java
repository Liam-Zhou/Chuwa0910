...
public class StreamApiIntermediateOperation {
    ...
    public void testFilter() {
        ...
        Stream<Employee>  e = employees.stream();
        e.map(Employee::getName).filter(name->name.length()>6).forEach(System.out::println);
    }
    ...
    public void testDistinct() {
        ...
        Arrays.asList(arr).stream().flatMapToInt(Arrays::stream).distinct().forEach(System.out::println);
    ...
    public void testFlatMap() {
        ...
        Stream<String> stringStream = listsOfStrings.stream().flatMap(Collection::stream);
        // 2.1.1 如果是使用普通map，则生成的是Stream<Stream>对象
        Stream<Stream<String>> streamStream = listsOfStrings.stream().map(Collection::stream);
        ...
        int sum = listsOfStrings.stream().flatMap(Collection::stream).mapToInt(Integer::parseInt).sum();
