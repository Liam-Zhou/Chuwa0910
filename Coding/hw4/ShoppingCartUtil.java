...
public class ShoppingCartUtil {
    ...
    public static Optional<String> getFirstItemName(Customer customer) {
        Optional<String> s = Optional.of(customer)
                .map(Customer::getShoppingCart)
                .map(ShoppingCart::getItems)
                .filter(items -> !items.isEmpty())
                .map((List<Item> lst) -> lst.get(0))
                .map(Item::getName);
    ...
    public static double getTotalPrice(Optional<Customer> customer) {
        ...
        double s = customer.map(Customer::getShoppingCart)
                .map(ShoppingCart::getItems)
                .filter(items -> !items.isEmpty())
    ...
    public static String getFirstItemNameWithAlternative(Customer customer) {
        String s = Optional.ofNullable(customer).map(Customer::getShoppingCart)
                .map(ShoppingCart::getItems)
                .filter(items -> !items.isEmpty())
    ...
    public static String getFirstItemNameOrThrowException(Customer customer) throws EmptyCartException {
        return Optional.ofNullable(customer)
                .map(ShoppingCartUtil::getFirstItemName).get()
                .orElseThrow();
    }
    ...
    public static void checkItemsInCart(Customer customer) {
        Optional<List<Item>> shoppingCart = Optional.ofNullable(customer)
                .map(Customer::getShoppingCart).map(ShoppingCart::getItems);

        if(shoppingCart.isPresent() && !shoppingCart.get().isEmpty()) {
            System.out.println("Has goods");
        } else {
    ...
    public static void printItemsInCart(Customer customer) {
        Optional<List<Item>> ListofItem = Optional.ofNullable(customer)
                .map(Customer::getShoppingCart).map(ShoppingCart::getItems);
        ListofItem.ifPresent(items -> {
            if (!items.isEmpty()) {
