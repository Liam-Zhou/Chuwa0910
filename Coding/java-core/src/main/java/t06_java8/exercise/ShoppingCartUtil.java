...
public class ShoppingCartUtil {
    ...
    public static Optional<String> getFirstItemName(Customer customer) {
        return Optional.ofNullable(customer)
                ...
                .filter(items -> !items.isEmpty()) // didn't come up with that
    ...
    public static double getTotalPrice(Optional<Customer> customer) {
        List<Item> itemsTmp = customer.map(Customer::getShoppingCart)
                .map(ShoppingCart::getItems)
                .filter(items -> !items.isEmpty())
                .orElse(new ArrayList<Item>());
        double sum = itemsTmp.stream()// really didn thought these two statements can be merged!
    ...
    public static String getFirstItemNameOrThrowException(Customer customer) throws EmptyCartException {
        return Optional.ofNullable(customer)
                ...
                .filter(items -> !items.isEmpty())
                .map(items -> items.get(0))
                .map(Item::getName) //just like things above
                .orElseThrow(() -> new EmptyCartException("购物车为空或不存在")); // that's how it's done
    }
    ...
    public static void checkItemsInCart(Customer customer) {
        String s = Optional.ofNullable(customer)
                .map(Customer::getShoppingCart)
                .map(ShoppingCart::getItems)
                .filter(items -> !items.isEmpty())
                .map(items -> "购物车中有商品")
                .orElse("购物车为空");// Yeah yeah I think that's doable!!
    ...
    public static void printItemsInCart(Customer customer) {
        List<String> list = Optional.ofNullable(customer)
                .map(Customer::getShoppingCart)
                .map(ShoppingCart::getItems)
                .filter(ITEMS -> !ITEMS.isEmpty())
        ...
        }                   // also doable but doesn use ifPresent
    }
